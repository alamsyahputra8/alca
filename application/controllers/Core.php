...
class Core extends CI_Controller {
	...
	public function insertpromo(){
		if(checkingsessionpwt()){
			...
			$rows = $this->query->insertData('promo', "kode_promo,name,sub,price,pict,date_start,date_end,status", "'$kode','$name','$sub','$price','$fileNamePost','$start','$end','$status'");
...
<tr>
	...
	public function insertuser(){
		if(checkingsessionpwt()){
			...
			$rows 		= $this->query->insertData('user', "userid,name,username,email,password,picture,id_role", "'$iduser','$name','$username','$email','$password','$fileNamePost','$role'");
	...
	public function insertmenus(){
		if(checkingsessionpwt()){
			...
			$rows 		= $this->query->insertData('menu_site', "id_menu,menu,description,background,link,style,parent,sort", 
													"'','$menu','$desc','$fileNamePost','$link','$style','$parent','$sort'");
			$id			= $this->db->insert_id();
			$url 		= "Manage Menus";
			$activity 	= "INSERT";
			if($rows) {
	...
	public function insertblog(){
		if(checkingsessionpwt()){
			...
			$q 			= "
						insert into blog (title,sub,id_menu,headline,link,content,picture,create_by,create_date) values ('$title','',$menu,'$headline','$link','$content','$fileNamePost','$userid','$date')
						";
						//echo $q;
			$rows 		= $this->query->insertDatabyQ($q);
	...
	public function insertlink(){
		if(checkingsessionpwt()){
			...
			$link		= $this->formula->clean(strtolower($title));
			$fileName 	= preg_replace("/[^a-zA-Z]/", "", time().$_FILES['pict']['name']);
			$fileName2 	= preg_replace("/[^a-zA-Z]/", "", time().$_FILES['schart']['name']);
			$date 		= date('Y-m-d');
			$rating		= trim(strip_tags(stripslashes($this->input->post('rating',true))));
			$sort		= trim(strip_tags(stripslashes($this->input->post('sort',true))));
			...
			$media 			= $this->upload->data();
			$fileNamePost 	= $media['file_name'];
			$this->makeThumbnails('link',$fileNamePost);

			$config2['upload_path'] = './images/other/'; //buat folder dengan nama assets di root folder
			...
			if(! $this->upload->do_upload('schart') )
			$this->upload->display_errors();
				 
			$media2 		= $this->upload->data();
			$fileNamePost2 	= $media2['file_name'];

			$q 			= "
						insert into link (title,link,picture,file,rating,sort) values ('$title','$link','$fileNamePost','$fileNamePost2','$rating','$sort')
						";
						//echo $q;
			$rows 		= $this->query->insertDatabyQ($q);
	...
	public function deletelink(){
		if(checkingsessionpwt()){
			...
			foreach ($coba as $dataex) {
				...
				$dataexis2 		= 'images/other/'.$dataex['file'];
				@unlink($dataexis);
				@unlink($dataexis2);
	...
	public function updatelink(){
		if(checkingsessionpwt()){
			...
			$cekinglogo	= $_FILES['upl']['name'];
			$cekinglog2	= $_FILES['ed_schart']['name'];
			$id			= trim(strip_tags(stripslashes($this->input->post('ed_id',true))));
			$title		= trim(strip_tags(stripslashes($this->input->post('ed_title',true))));
			$rating		= trim(strip_tags(stripslashes($this->input->post('ed_rating',true))));
			$sort		= trim(strip_tags(stripslashes($this->input->post('ed_sort',true))));
			// $link		= trim(strip_tags(stripslashes($this->input->post('ed_link',true))));
			$link		= $this->formula->clean(strtolower($title));
			...
			if ($cekinglogo!='') {
				...
				$rows = $this->query->updateData('link',"title='$title', link='$link', picture='$fileNamePost', rating='$rating', sort='$sort'","WHERE id_link='$id'");
			} else if ($cekinglog2!='') {
				//delete eksisting
				$coba = $this->query->getData('link','file','WHERE id_link='.$id.'');
				foreach ($coba as $dataex) {
					$dataexis 		= 'images/other/'.$dataex['file'];
					unlink($dataexis);
				}

				$fileName2 	= preg_replace("/[^a-zA-Z]/", "", time().$_FILES['ed_schart']['name']);
				$config2['upload_path'] = './images/other/'; //buat folder dengan nama assets di root folder
				$config2['file_name'] = $fileName2;
				$config2['allowed_types'] = 'gif|jpg|png|jpeg|JPG';
				$config2['max_size'] = 10000000;
				 
				$this->load->library('upload');
				$this->upload->initialize($config2);
				 
				if(! $this->upload->do_upload('ed_schart') )
				$this->upload->display_errors();
				...
				$rows = $this->query->updateData('link',"title='$title', link='$link', file='$fileNamePost2', rating='$rating', sort='$sort'","WHERE id_link='$id'");
			} else {
				$rows = $this->query->updateData('link',"title='$title', link='$link', rating='$rating', sort='$sort'","WHERE id_link='$id'");
			}
	...
	public function insertbanner(){
		if(checkingsessionpwt()){
			...
			$q 			= "
						insert into banner (title,sub,img,thumb) values ('$title','$sub','$fileNamePost','blur_thumb.png')
						";
						//echo $q;
			$rows 		= $this->query->insertDatabyQ($q);
		...
		if(checkingsessionpwt()){
			...
			for($y = 0; $y < $jmlImg; $y++){
				...
				if (!empty($lokasi_file[$y])){
					...
					$q 			= "insert into photos (id_photo,id_menu,id_album,picture) values ('','$album','$album','$nama_file_unik[$y]')";
					$rows 		= $this->query->insertDatabyQ($q);
				}
			}
	...
	public function insertservices(){
		if(checkingsessionpwt()){
			...
			$q 			= "
						insert into services (title,sub,id_menu,headline,picture,link,content) values ('$title','',$menu,'$headline','$fileNamePost','$link','$content')
						";
						//echo $q;
			$rows 		= $this->query->insertDatabyQ($q);
	...
	public function regeneratethumb() {
		$getData 	= $this->query->getDatabyQ("select * from link");
		foreach ($getData as $data) {
			// echo $data['picture'].'<br>';
			$this->makeThumbnails('link',$data['picture']);
		}
	}
	...
	public function insertalbum(){
		if(checkingsessionpwt()){
			...
			$q 			= "
						insert into album (title,id_menu,icon,id_services,link) values ('$title',$works,'$fileNamePost','$services','$link')
						";
						//echo $q;
			$rows 		= $this->query->insertDatabyQ($q);
	...
	public function loadmoreEvent(){
        ...
        $q 					= "
        					SELECT * FROM  event_detail order by 1 desc LIMIT $start,$content_per_page
        					";
	        ...
	        echo '<div id="portfolio" class="grid-layout portfolio-3-columns" data-margin="20">';
	            foreach ($all_content as $data) :
	            	...
	            	echo ' <div class="portfolio-item img-zoom ct-foto">
		                        <div class="portfolio-item-wrap">
		                            <div class="portfolio-image">
		                                <a href="#"><img src="'.base_url().'images/event/'.$file.'" alt=""></a>
		                            </div>
		                            <div class="portfolio-description">
	...
	public function loadmoreBrand(){
        ...
        $q 					= "
        					SELECT * FROM  link order by sort asc, id_link desc LIMIT $start,$content_per_page
        					";
        $all_content 		= $this->query->getDatabyQ($q);
        if(isset($all_content) && is_array($all_content) && count($all_content)) : 
	    	...
	    	echo '
			<div class="container">';
	        echo '<ul class="grid grid-4-columns">';
	            foreach ($all_content as $data) :
	            	$file 			= $data['picture'];
	            	$ratingbrand    = $data['rating'];
	            	echo '	<li>
	            				<div class="product">
			                        <a href="'.base_url().'shop/brand/'.$data['link'].'" title="'.$data['title'].'"><img alt="" src="'.base_url().'images/link/'.$data['picture'].'"> </a>
			                        <div class="product-rate">';
		                            for($i=0; $i<$ratingbrand; $i++) {
			                            echo '<i class="fa fa-star"></i>';
			                        }
			                        echo '</div>
			                        <div class="product-reviews"><a href="'.base_url().'shop/brand/'.$data['link'].'" style="width: auto!important;" class="btn btn-dark btn-xs">SHOP NOW</a></div>
			                    </div>
	            			</li>
	            		';
	            endforeach;
	...
	public function getdataproduct(){
		if(checkingsessionpwt()){
			...
			$arraynya	= $columnsDefault;
			$jsonfile	= base_url().'jsondata/dataproduct';
	...
	public function insertproduct(){
		if(checkingsessionpwt()){
			...
			$query = $this->query->getData('product','max(id_product)+1 as id_product','');
			$getID = array_shift($query);
			if ($getID['id_product']=='') {
				$id = '1';
			} else {
			...
			$diskon2	= trim(strip_tags(stripslashes($this->input->post('disc2',true))));
			$category	= trim(strip_tags(stripslashes($this->input->post('category',true))));
			$description= trim(strip_tags(stripslashes($this->input->post('description',true))));
			$brand 		= trim(strip_tags(stripslashes($this->input->post('brand',true))));
			$weight 	= trim(strip_tags(stripslashes($this->input->post('weight',true))));
			$popular 	= trim(strip_tags(stripslashes($this->input->post('popular',true))));
			
			$fileName 	= preg_replace("/[^a-zA-Z]/", "", time().$_FILES['pict']['name']);
				
			$config['upload_path'] 		= './images/product/'; //buat folder dengan nama assets di root folder
			$config['file_name'] 		= 'CVR_'.$fileName;
			$config['allowed_types'] 	= 'gif|jpg|png|jpeg|JPG';
			...
			if($rows) {
				// PRODUCT COVER
				$qCVR 		= "
							insert into product_image (id_product,file,cover) values 
							('$id','$fileNamePost','1')
							";
							//echo $q;
				$rowsCVR	= $this->query->insertDatabyQ($qCVR);

				// PRODUCT STOCK
				$stokAL 	= $this->input->post('stockA',true);
				$stokS 		= $this->input->post('stockS',true);
				$stokM 		= $this->input->post('stockM',true);
				$stokL 		= $this->input->post('stockL',true);
				$stokXL 	= $this->input->post('stockXL',true);
				$stokXXL 	= $this->input->post('stockXXL',true);

				$qAL 		= "insert into product_stok (id_product,warna,size,stok) values ('$id','','ALL SIZE','$stokAL')";
				$rowsAL 	= $this->query->insertDatabyQ($qAL);

				$qS 		= "insert into product_stok (id_product,warna,size,stok) values ('$id','','S','$stokS')";
				$rowsS 		= $this->query->insertDatabyQ($qS);

				$qM 		= "insert into product_stok (id_product,warna,size,stok) values ('$id','','M','$stokM')";
				$rowsM 		= $this->query->insertDatabyQ($qM);

				$qL 		= "insert into product_stok (id_product,warna,size,stok) values ('$id','','L','$stokL')";
				$rowsL 		= $this->query->insertDatabyQ($qL);

				$qXL 		= "insert into product_stok (id_product,warna,size,stok) values ('$id','','XL','$stokXL')";
				$rowsXL 	= $this->query->insertDatabyQ($qXL);

				$qXXL 		= "insert into product_stok (id_product,warna,size,stok) values ('$id','','XXL','$stokXXL')";
				$rowsXXL 	= $this->query->insertDatabyQ($qXXL);
	...
	public function insertimageproduct(){
		if(checkingsessionpwt()){
			...
			$id			= trim(strip_tags(stripslashes($this->input->post('idproduct',true))));

			$url 		= "Manage Product Image";
			$activity 	= "INSERT";
	...
	public function uploadProductImg(){
		if(checkingsessionpwt()){
			...
			if (!empty($_FILES)) {
			    ...
			    $config['upload_path'] 		= './images/product/'; //buat folder dengan nama assets di root folder
				$config['file_name'] 		= 'PRD_'.$fileName;
				...
				$qED 		= "insert into product_image (id_product,file,cover) values ('$id','$fileNamePost','0')";
				$insertED 	= $this->query->insertDatabyQ($qED);
	...
	public function uploadProductImgMore($id){
		if(checkingsessionpwt()){
			$ds          = DIRECTORY_SEPARATOR;
	...
	public function modalproductimg(){
		if(checkingsessionpwt()){
	...
	public function deleteimg_product(){
		if(checkingsessionpwt()){
			$url 		= "Manage Product Image";
			$activity 	= "DELETE";
			...
			$coba = $this->query->getData('product_image','file','WHERE id_image='.$cond.'');
			foreach ($coba as $dataex) {
				$dataexis 	= 'images/product/'.$dataex['file'];
				$dataexisT 	= 'images/product/thumb_'.$dataex['file'];
				@unlink($dataexis);
				@unlink($dataexisT);
			}

			$rows = $this->query->deleteData('product_image','id_image',$cond);
	...
	public function deleteproduct(){
		if(checkingsessionpwt()){
			...
			//delete eksisting
			$coba = $this->query->getData('product_image','file','WHERE id_product='.$cond.'');
			foreach ($coba as $dataex) {
				$dataexis 		= 'images/product/'.$dataex['file'];
				@unlink($dataexis);
			}

			$rows 	= $this->query->deleteData('product','id_product',$cond);
			$rows2 	= $this->query->deleteData('product_image','id_product',$cond);
			$rows3 	= $this->query->deleteData('product_stok','id_product',$cond);
			$rows4 	= $this->query->deleteData('product_rating','id_product',$cond);
		...
		if(checkingsessionpwt()){
			...
			$q 				= "
							select
		                        ...
		                        (select stok from product_stok where id_product=a.id_product and size='ALL SIZE') stokALL,
		                        (select stok from product_stok where id_product=a.id_product and size='S') stokS,
		                        (select stok from product_stok where id_product=a.id_product and size='M') stokM,
		                        (select stok from product_stok where id_product=a.id_product and size='L') stokL,
		                        (select stok from product_stok where id_product=a.id_product and size='XL') stokXL,
		                        (select stok from product_stok where id_product=a.id_product and size='XXL') stokXXL,
								(SELECT xb.name as  update_by FROM `data_log` xa LEFT JOIN user xb ON xa.userid=xb.userid 
	...
	public function updateproduct(){
		if(checkingsessionpwt()){
			...
			$description= trim(strip_tags(stripslashes($this->input->post('ed_description',true))));
			$brand 		= trim(strip_tags(stripslashes($this->input->post('ed_brand',true))));
			$diskon 	= trim(strip_tags(stripslashes($this->input->post('ed_disc',true))));
			$diskon2 	= trim(strip_tags(stripslashes($this->input->post('ed_disc2',true))));
			$weight 	= trim(strip_tags(stripslashes($this->input->post('ed_weight',true))));
			$popular 	= trim(strip_tags(stripslashes($this->input->post('ed_popular',true))));
			...
			if ($cekinglogo!='') {
				//delete eksisting
				$coba = $this->query->getData('product_image','file','WHERE id_product='.$id.' and cover=1');
				foreach ($coba as $dataex) {
					$dataexis = 'images/product/'.$dataex['file'];
				}
				unlink($dataexis);
				...
				$qPI 		= $this->query->updateData('product_image',
							"file='$fileNamePost'",
			...
			// UPDATE STOK PRODUCT
			$delSTOK 	= $this->query->deleteData('product_stok','id_product',$id);

			$stokAL 	= $this->input->post('ed_stockA',true);
			$stokS 		= $this->input->post('ed_stockS',true);
			$stokM 		= $this->input->post('ed_stockM',true);
			$stokL 		= $this->input->post('ed_stockL',true);
			$stokXL 	= $this->input->post('ed_stockXL',true);
			$stokXXL 	= $this->input->post('ed_stockXXL',true);
	...
	public function updatediskonperbrand(){
		if(checkingsessionpwt()){
			$userdata	= $this->session->userdata('sesspwt'); 
	...
	public function loadmoreProduct(){
        ...
        if ($type=='list') { $wheretype = "and linkmenu='$id'"; }
        else if ($type=='product') { $wheretype = "and linkcategory='$id'"; }
        else if ($type=='brand') { $wheretype = "and linkbrand='$id'"; }
        else if ($type=='promo') { $wheretype = "and (diskon>'0' or diskon_special>'0')"; }
        else if ($type=='home') { $wheretype = "and popular_product='1'"; }
        else { $wheretype= ""; }

        if ($category=='ALL' or $category=='') { $wherecat = ''; } else { $wherecat = "and linkcategory='$category'"; }
        if ($brand=='ALL' or $brand=='') { $wherebrand = ''; } else { $wherebrand = "and id_link='$brand'"; }
        if ($size=='ALL' or $size=='') { $wheresize = ''; } else { $wheresize = "and EXISTS (select * from product_stok where size='$size')"; }
        
        if ($price=='LTH') { $orderby = "order by pricefinal asc"; }
        else if ($price=='HTL') { $orderby = "order by pricefinal desc"; }
        else { 
        	if ($type=='home') {
        		$orderby = "order by popular_product desc, last_update_date desc"; 
        	} else {
        ...
        $q 					= "
        					select * from (
                                select
                                    a.*,
                                    case when diskon>0 then round(price-diskon) else price end pricefinal,
                                    (select file from product_image where id_product=a.id_product and cover='1' limit 1) cover,
                                    ...
                                    (SELECT DATE_FORMAT(xa.date_time, '%d-%b-%y %H:%i:%s') as last_update FROM `data_log` xa LEFT JOIN user xb ON xa.userid=xb.userid 
                                    WHERE xa.menu='Manage Product' AND xa.data = a.id_product ORDER BY xa.date_time DESC limit 1) as last_update,
                                    (SELECT xa.date_time as last_update_date FROM `data_log` xa LEFT JOIN user xb ON xa.userid=xb.userid 
                                    WHERE xa.menu='Manage Product' AND xa.data = a.id_product ORDER BY xa.date_time DESC limit 1) as last_update_date
                                from
                                product a
                            ) as final
                            where 1=1 $wheretype $wherecat $wherebrand $wheresize
                            $orderby
        ...
        //                 <div><img src="'.base_url().'images/icon/notfound.png"></div><br>
        ...
        // } else {
	        if(isset($all_content) && is_array($all_content) && count($all_content)) : 
		    	...
		    	echo '
		    	<style>
		            #productbase .product .product-price {
		                width: 100%;
		                margin-bottom: 5px;
		                margin-top: 5px;
		                text-align: left;
		            ...
		            #productsmallsize.product-size li label span {
					    cursor: pointer;
					    display: block;
					    font-size: 12px;
					    font-weight: 400;
				...
				<div id="productbase" class="container" style="padding: 0px!important;">
					<div class="shop">';
			        echo '<div class="grid-layout grid-4-columns" data-item="grid-item">';
			            foreach ($all_content as $data) :
			            	$id 	= $data['id_product'];
			            	$qSize 	= "SELECT a.*, (select sum(stok) from product_stok where size=a.size and id_product=$id) jml FROM `product_size` a order by sort";
			            	$dSize 	= $this->query->getDatabyQ($qSize);

			            	$file 	= $data['cover'];
			            	if ($data['diskon']>0 or $data['diskon_special']>0) {
			            			...
			            			<div class="grid-item">
		                                <div class="product">
		                                    <div class="product-image">
		                                        <a href="'.base_url().'product/'.$data['link'].'">
		                                            <img alt="'.$data['name'].'" src="'.base_url().'images/product/thumb_'.$data['cover'].'">
		                                        </a>
		                                        <a href="'.base_url().'product/'.$data['link'].'">
		                                            <img alt="'.$data['name'].'" src="'.base_url().'images/product/thumb_'.$data['back'].'">
		                                        </a>
		                                        '.$tagdiskon.'
		                                        <!--div class="product-overlay">
		                                            <a href="'.base_url().'quickview/'.$data['link'].'" data-lightbox="ajax">Quick View</a>
		                                        </div-->
		                                    </div>
		                                    ...
		                                    <div class="product-description">
		                                        ...
		                                        <!--div class="product-category">'.$data['nametype'].' - '.$data['namecategory'].'</div-->
		                                        <div class="product-price">'.$diskon.'<ins>IDR. '.$price.'</ins></div>
		                                        <ul id="productsmallsize" class="product-size">';
		                                        foreach ($dSize as $size) { 
		                                        	...
		                                        	echo '
		                                        	<li>
		                                                <button class="btn btn-'.$color.' btn-xs">'.$labelsize.'</button>
		                                            </li>
		                                        	';
		                                        }
		                                        echo '
				...
				<!--Template functions-->
				<script src="'.base_url().'assets/polo/js/product.js"></script>
				<script>
	...
	public function getCity() {
		...
		curl_setopt_array($curl, array(
		  CURLOPT_URL => "https://api.rajaongkir.com/starter/city?id=&province=$province",
		  CURLOPT_RETURNTRANSFER => true,
		  CURLOPT_ENCODING => "",
		  CURLOPT_MAXREDIRS => 10,
		  CURLOPT_TIMEOUT => 30,
	...
	public function getdataproducttype(){
		if(checkingsessionpwt()){
			...
			$arraynya	= $columnsDefault;
			$jsonfile	= base_url().'jsondata/dataproducttype';
	...
	public function insertproducttype(){
		if(checkingsessionpwt()){
			...
			$q 			= "
						insert into product_type (id_type,name,id_menu,sort) values 
						('','$name','$menu', '')
						";
						//echo $q;
			$rows 		= $this->query->insertDatabyQ($q);
	...
	public function deleteproducttype(){
		if(checkingsessionpwt()){
			...
			$rows 	= $this->query->deleteData('product_type','id_type',$cond);
	...
	public function modalproducttype(){
		if(checkingsessionpwt()){
			
			$id				= trim(strip_tags(stripslashes($this->input->post('id',true))));
			
			$q 				= "
							select a.*, (select menu from menu_site where id_menu=a.id_menu) menu from product_type a
							where a.id_type='".$id."'
							";
			$getData		= $this->query->getDatabyQ($q);
	...
	public function updateproducttype(){
		if(checkingsessionpwt()){
			...
			// UPDATE BASE PRODUCT
			$rows 	= $this->query->updateData('product_type',
					"name='$name', id_menu='$menu'",
					"WHERE id_type='$id'");
	...
	public function getdataproductCat(){
		if(checkingsessionpwt()){
			...
			$arraynya	= $columnsDefault;
			$jsonfile	= base_url().'jsondata/dataproductCat';
	...
	public function insertproductCat(){
		if(checkingsessionpwt()){
			...
			$q 			= "
						insert into product_category (id_category,name,link,id_type) values 
						('','$name','$link','$type')
						";
						//echo $q;
			$rows 		= $this->query->insertDatabyQ($q);
	...
	public function deleteproductCat(){
		if(checkingsessionpwt()){
			...
			$rows 	= $this->query->deleteData('product_category','id_category',$cond);
	...
	public function modalproductCat(){
		if(checkingsessionpwt()){
	...
	public function updateproductCat(){
		if(checkingsessionpwt()){
			...
			// UPDATE BASE PRODUCT
			$rows 	= $this->query->updateData('product_category',
					"name='$name', id_type='$type'",
	...
	public function modalstream(){
		if(checkingsessionpwt()){
			
			$id					= trim(strip_tags(stripslashes($this->input->post('id',true))));
			
			$getData			= $this->query->getData('streaming','*',"WHERE id_stream='".$id."'");
	...
	public function getdataStream() {
		if(checkingsessionpwt()){
			$getData 	= $this->query->getData('streaming','*',"");
			...
			echo '
				<div class="row">
                    <div class="col-lg-12">
                        <iframe style="width: 100%;" height="720" src="'.$embed.'?rel=0&amp;showinfo=0" allowfullscreen></iframe>
                    </div>
                </div>
	...
	public function updatestream(){
		if(checkingsessionpwt()){
			...
			// UPDATE BASE PRODUCT
			$rows 	= $this->query->updateData('streaming',
