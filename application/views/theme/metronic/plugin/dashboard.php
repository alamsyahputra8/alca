<?PHP
$getDC  = $this->query->getData('konter','*',"ORDER BY tanggal ASC");
?>
<script>
"use strict";

/*KTApp.block('#visitorcounter .loadervc', {
    overlayColor: '#000000',
    type: 'v2',
    state: 'success',
    message: 'Please wait...'
...
var KTDatatablesSearchOptionsColumnSearch = function() {
    ...
    var initTable1 = function() {
        ...
        var table = $('#tabledata').DataTable({
            ...
            ajax: {
                url: '<?PHP echo base_url(); ?>core/getdatalog',
                type: 'POST',
                data: {
                    // parameters for custom backend script demo
                    columnsDef: [
                        'date', 'photo', 'user', 'activity', 'id', 'data', 'menu', ],
                },
            },
            columns: [
                {data: 'date'},
    ...
    var initTable2 = function() {
        ...
        var table = $('#tableinbox').DataTable({
            ...
            //ajax: '<?PHP echo base_url(); ?>core/getdatauser',
            ajax: {
                url: '<?PHP echo base_url(); ?>core/getdatainboxhome',
                type: 'POST',
                data: {
                    // parameters for custom backend script demo
                    columnsDef: [
...
var KTamChartsChartsDemo = function() {
    var visitor = function() {
        var chart = AmCharts.makeChart("visitor", {
            ...
            "dataProvider": [
            <?PHP 
            foreach ($getDC as $dataCount) { 
                $qVal   = $this->query->getData('konter','count(ip) jml',"WHERE tanggal='".$dataCount['tanggal']."'");
                $value  = array_shift($qVal);
            ?>
            {
                "date": "<?PHP echo $dataCount['tanggal']; ?>",
                "value": <?PHP echo $value['jml']; ?>
            },
            <?PHP } ?>
            ]
        });
