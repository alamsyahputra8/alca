...
$(document).on('click', '.btnupdateM', function(e){
    ...
    $.ajax({
        url: '<?PHP echo base_url(); ?>core/modalalbum',
        type: 'POST',
        data: 'id='+uid,
        dataType: 'json'
    })
    .done(function(data){
        ...
        $('#ed_id').val(data.id_album);
        $('#ed_title').val(data.title);
        $('#ed_works').val(data.id_menu);
        $('#ed_works').trigger('change.select2');
        $('#ed_service').val(data.id_services);
        $('#ed_service').trigger('change.select2');

        //$('#ed_content').val(data.content);
        //$('#ed_content').summernote('pasteHTML', data.content);
...
var KTDatatablesSearchOptionsColumnSearch = function() {
	...
	var initTable1 = function() {
        ...
        var table = $('#tabledata').DataTable({
            ...
            ajax: {
                url: '<?PHP echo base_url(); ?>core/getdataalbum',
                type: 'POST',
                data: {
                    // parameters for custom backend script demo
                    columnsDef: [
                        'icon', 'title', 'works', 'updateby', 'lastupdate', 'actions',],
                },
            },
			columns: [
                {data: 'icon'},
...
var KTFormWidgets = function () {
    ...
    var initInsert = function () {
    	$('#saveinsert').click(function(e) {
            ...
            form.ajaxSubmit({
                url: "<?PHP echo base_url(); ?>core/insertalbum",
                type: "POST",
                beforeSend: function(){ 
    ...
    var initUpdate = function () {
        $('#saveupdate').click(function(e) {
            ...
            form.ajaxSubmit({
                url: "<?PHP echo base_url(); ?>core/updatealbum",
                type: "POST",
                beforeSend: function(){ 
    ...
    var initDelete = function () {
    	$('#deleteBtn').click(function(e) {
            ...
            form.ajaxSubmit({
                url: "<?PHP echo base_url(); ?>core/deletealbum",
                type: "POST",
                success: function(data) {
                    if(data) {
                        // similate 2s delay
