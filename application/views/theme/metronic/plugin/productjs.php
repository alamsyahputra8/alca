...
$(document).on('click', '.btnupdateM', function(e){
    ...
    $.ajax({
        url: '<?PHP echo base_url(); ?>core/modalproduct',
        type: 'POST',
        data: 'id='+uid,
        dataType: 'json'
    })
    .done(function(data){
        ...
        $('#ed_weight').val(data.weight);
        $('#ed_brand').val(data.id_link);
        $('#ed_brand').trigger('change.select2');
        $('#ed_category').val(data.id_category);
        $('#ed_category').trigger('change.select2');
        $('#ed_stockA').val(data.stokALL);
        $('#ed_stockS').val(data.stokS);
        $('#ed_stockM').val(data.stokM);
        $('#ed_stockL').val(data.stokL);
        $('#ed_stockXL').val(data.stokXL);
        $('#ed_stockXXL').val(data.stokXXL);
        $('#ed_description').summernote('code', data.description);
        if (data.popular_product==1) {
            $('#ed_popular').attr("checked",true);
        } else {
            $('#ed_popular').attr("checked",false);
        }
...
var KTDatatablesSearchOptionsColumnSearch = function() {
	...
	var initTable1 = function() {
        ...
        var table = $('#tabledata').DataTable({
            ...
            ajax: {
                url: '<?PHP echo base_url(); ?>core/getdataproduct',
                type: 'POST',
                data: {
                    // parameters for custom backend script demo
                    columnsDef: [
                        'cover', 'name', 'type', 'category', 'brand', ,'price', 'popular', 'updateby', 'lastupdate', 'actions',],
                },
            },
			columns: [
                {data: 'cover'},
			...
			columnDefs: [
				{
                    ...
                    render: function(data, type, full, meta) {
                        if (aksesUpdate=='ada') {
                            ...
                            <a href="<?PHP echo base_url(); ?>panel/product-image/`+data+`" class="btn btn-sm btn-clean btn-icon btn-icon-md btnupdateIP" title="Image Product" data-id="`+data+`">
                                <i data-toggle="tooltip" title="Image Product" class="la la-camera-retro"></i>
                            </a>
                            `;
                        } else {
                            var z = ``;
...
var KTFormWidgets = function () {
    ...
    var initInsert = function () {
    	$('#saveinsert').click(function(e) {
            ...
            form.ajaxSubmit({
                url: "<?PHP echo base_url(); ?>core/insertproduct",
                type: "POST",
                beforeSend: function(){ 
    ...
    var initUpdate = function () {
        $('#saveupdate').click(function(e) {
            ...
            form.validate({
                rules: {
                    ed_name: {required: true},
                    ed_pict: {required: true},
                    ed_price: {required: true},
                    ed_diskon: {required: true},
                    ed_diskon2: {required: true},
                    ed_category: {required: true},
                    ed_brand: {required: true},
                    ed_description: {required: true},
                }
            });
            ...
            form.ajaxSubmit({
                url: "<?PHP echo base_url(); ?>core/updateproduct",
                type: "POST",
                beforeSend: function(){ 
    ...
    var initDiskon = function () {
        $('#saveconf').click(function(e) {
            ...
            form.ajaxSubmit({
                url: "<?PHP echo base_url(); ?>core/updatediskonperbrand",
                type: "POST",
                beforeSend: function(){ 
                   KTApp.block('#configdiskon .modal-content', {
                        overlayColor: '#000000',
                        type: 'v2',
                        state: 'success',
                        message: 'Please wait...'
                ...
                success: function(data) {
                    if(data) {
                        ...
                        setTimeout(function() {
                            btn.removeClass('kt-spinner kt-spinner--right kt-spinner--sm kt-spinner--light').attr('disabled', false);
                            //showErrorMsg(form, 'success', '<strong>Data Insert Success!</strong>');
                            KTApp.unblock('#configdiskon .modal-content');
                            
                            $('#configdiskon').modal('toggle');
                            $('#tabledata').DataTable().ajax.reload();
                            $('#formconf')[0].reset();
                            var alert = $('#suksesinsert');
                            alert.removeClass('kt-hidden').show();
    ...
    var initDelete = function () {
    	$('#deleteBtn').click(function(e) {
            ...
            form.ajaxSubmit({
                url: "<?PHP echo base_url(); ?>core/deleteproduct",
                type: "POST",
                success: function(data) {
                    if(data) {
                        // similate 2s delay
