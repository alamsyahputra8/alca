...
<?PHP $this->load->view('theme/polo/plugin1'); ?>
        ...
        $(document).ready(function() {
            ...
            $('#results').load("<?php echo base_url() ?>core/loadmoreEvent", {'group_no':total_record}, function() {$('#loaderpage').fadeOut(); total_record++;});
            ...
            $(document).on('click', '.btnloadmore', function(e){
                    if(total_record <= total_groups) {
                      loading = true; 
                      $('#loaderpage').fadeIn(); 
                      $.post('<?PHP echo site_url() ?>core/loadmoreEvent',{'group_no': total_record},
                        function(data){ 
                            if (data != "") {
                                $("#results").append(data);                 
                                $('#loaderpage').fadeOut();
